...
var combi = [64, 270, 31, 90, 44, 85, 67, 300]; //pass combination
var combiPosition = 0;
...
.map(n => xinput.WrapController(n, { interval: 15, deadzone: { x: 0.18, y: 0.18 }, holdtime: 500 }))
.forEach(gamepad => {
  ...
  gamepad.addListener("analog-input", (input, data) => { //what's beautiful about his wrapper is that it outputs both axes simultaneously
    if(input == "rightstick") {
      ...
      if(angle == combi[combiPosition]) { 
        combiPosition++;
          match = true;
      }
      
      logupd(`${padEnd(angle, 3)}° | ${padEnd(distance, 3)} | ${dirSym} | ${combiPosition + 1} | ` + (match ? "🔓 Tchik!" : "🔒"));

      if(combiPosition == combiPosition.length) {} else match = false;
    }
	});
});